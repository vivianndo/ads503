...
#### <br><b>2.2 Remove features</b><br><br>

```{r}
df <- subset(df, select = -c(encounter_id, patient_nbr,weight,medical_specialty,payer_code,diag_1,diag_2,diag_3,race))
dim(df)
sum(is.na(df))
```

Feature deletions were performed on the following variables for the following reasons:
...
After feature deletion, there 23 variables remaining--22 predictors and 1 response variable.
...
df_dummies <- dummy_cols(df, 
           select_columns=c("gender","age","admission_type_id","discharge_disposition_id", "admission_source_id", "A1Cresult", "metformin", "glipizide", "glyburide","pioglitazone", "rosiglitazone", "insulin", "change","diabetesMed"),
...
```{r}
# Separate X and Y
dfX <- subset(df_dummies, select = -c(readmitted))
dfY <- subset(df_dummies, select = c(readmitted))
...
ctrl <- trainControl(method="cv",summaryFunction = twoClassSummary, classProbs=TRUE, savePredictions="final")
set.seed(123)

# Logistic Regression 
set.seed(123)
lrFit <- train(x=trainX,y=trainY,method="glm",metric="ROC", trControl=ctrl)

# Linear Discriminant Analysis
set.seed(123)
ldaFit <- train(x=trainX,y=trainY,method="lda",metric="ROC",trControl=ctrl)

# Penalized Logistic Regression
set.seed(123)
glmnGrid <- expand.grid(alpha=c(0,0.1,0.2,0.4,0.6,0.8,1),
                        lambda=seq(.01, .2, length=10))
glmnFit <- train(x=trainX,y=trainY,method="glmnet",tuneGrid=glmnGrid, metric="ROC", trControl=ctrl)

# Nearest Shrunken Centroids
set.seed(123)
nscFit <- train(x=trainX,y=trainY,method="pam",preProcess=c("center", "scale"),tuneGrid=data.frame(threshold=seq(0,25, length=30)),metric="ROC",trControl=ctrl)

## Compare model performance ##
# Create a data frame to store all test results 
testResults<- data.frame(obs=trainY,
                          LR=predict(lrFit,trainX),
                          LDA=predict(ldaFit,trainX),
                          PenalizedLR=predict(glmnFit,trainX),
                          NSC=predict(nscFit,trainX))
# Confusion matrix
lrFitCM <- confusionMatrix(lrFit, norm="none")
glmnetCM <- confusionMatrix(glmnFit, norm="none")
ldaFitCM <- confusionMatrix(ldaFit, norm="none")
nscFitCM <- confusionMatrix(nscFit, norm="none")
...
legend("bottomright", legend=c("LR", "GLMNET","LDA", "NSC"), col=c("red","green", "blue", "black"), lwd=2)
