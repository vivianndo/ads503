...
```{r}
# df_pr <- subset(df_pr, select = -c(encounter_id, patient_nbr,weight,medical_specialty,payer_code,diag_1,diag_2,diag_3,race))
...
# Remove outlier from column 'number_diagnoses'
...
# Remove outlier from column 'number_inpatient'
# quartiles <- quantile(df_pr$num_lab_procedures, probs=c(.25, .75), na.rm = FALSE)
# IQR <- IQR(df_pr$num_lab_procedures)
# Lower <- quartiles[1] - 2*IQR
# Upper <- quartiles[2] + 2*IQR 
# count(df_pr$num_lab_procedures > Lower & df_pr$num_lab_procedures < Upper)
# 
# df_pr <- subset(df_pr, df_pr$num_lab_procedures > Lower & df_pr$num_lab_procedures < Upper)
# dim(df_pr)
...
#### <br><b>2.5 Separate X and Y</b><br><br>
...
#### <br><b>2.6 Convert categorical variables to dummy variables</b><br><br>
...
* 99797 objects with 72 variables will be used for modeling.

#### <br><b>2.7 Partition data into training and test datasets using a 70% ratio</b><br><br>
...
metrics_tr <- data.frame(Metric.Train = c("Accuracy", "Sensitivity", "Specificity", "Precision")) 

metrics_tr$LR <- get_training_metrics(lrFit)
#metrics_tr
...
#metrics_test
cat("LR (training): accuracy =", metrics_tr$LR[1], ", sensitivity =", metrics_tr$LR[2], 
    " , specificity =", metrics_tr$LR[3], ", Precision =", metrics_tr$LR[4], "\n")

cat("LR (test): accuracy =", metrics_test$LR[1], " , sensitivity =", metrics_test$LR[2], 
    ", specificity =", metrics_test$LR[3], ", Precision =", metrics_tr$LR[4], "\n")
...
#ldaFit
# Calculate training/resampling performance metrics
metrics_tr$LDA <- get_training_metrics(ldaFit)
#metrics_tr
...
# Display model performance
cat("LDA (training): accuracy =", metrics_tr$LDA[1], ", sensitivity =", metrics_tr$LDA[2], 
    " , specificity =", metrics_tr$LDA[3], ", Precision =", metrics_tr$LDA[4], "\n")

cat("LDA (test): accuracy =", metrics_test$LDA[1], " , sensitivity =", metrics_test$LDA[2], 
    ", specificity =", metrics_test$LDA[3], ", Precision =", metrics_tr$LDA[4], "\n")
...
# Calculate training/resampling performance metrics
metrics_tr$GLMN <- get_training_metrics(glmnFit)
#metrics_tr
...
# Display model performance
cat("Penalized LR (training): accuracy =", metrics_tr$GLMN[1], ", sensitivity =", metrics_tr$GLMN[2], 
    " , specificity =", metrics_tr$GLMN[3], ", Precision =", metrics_tr$GLMN[4], "\n")

cat("Penalized LR (test): accuracy =", metrics_test$GLMN[1], " , sensitivity =", metrics_test$GLMN[2], 
    ", specificity =", metrics_test$GLMN[3], ", Precision =", metrics_tr$GLMN[4], "\n")
...
# Calculate training/resampling performance metrics
metrics_tr$NSC <- get_training_metrics(nscFit)
...
# Display model performance
cat("NSC (training): accuracy =", metrics_tr$NSC[1], ", sensitivity =", metrics_tr$NSC[2], 
    " , specificity =", metrics_tr$NSC[3], ", Precision =", metrics_tr$NSC[4], "\n")

cat("NSC (test): accuracy =", metrics_test$NSC[1], " , sensitivity =", metrics_test$NSC[2], 
    ", specificity =", metrics_test$NSC[3], ", Precision =", metrics_tr$NSC[4], "\n")
...
# Calculate training/resampling performance metrics
metrics_tr$MDA <- get_training_metrics(mdaFit)
#metrics_tr
...
# Display model performance
cat("MDA (training): accuracy =", metrics_tr$MDA[1], ", sensitivity =", metrics_tr$MDA[2], 
    " , specificity =", metrics_tr$MDA[3], ", Precision =", metrics_tr$MDA[4], "\n")

cat("MDA (test): accuracy =", metrics_test$MDA[1], " , sensitivity =", metrics_test$MDA[2], 
    ", specificity =", metrics_test$MDA[3], ", Precision =", metrics_tr$MDA[4], "\n")
...
#### <br><b>3.6 Support Vector Machines</b><br><br>

* Tried, but had to stop running. It is too slow!
...
# 
# set.seed(123)
# svmRFit <- train(x = trainX, y = trainY,
...
mdaROC <- roc(response=trainY,predictor=mdaFit$pred$YES,levels=rev(levels(nscFit$pred$obs)))
...
metrics_tr <- rbind(metrics_tr, c("AUC", lrAUC, glmnAUC, ldaAUC, nscAUC, mdaAUC))
